...
//    @PostMapping("/login")
    public String login(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletResponse response) {
        ...
        Member loginMemer = loginService.login(form.getLoginId(), form.getPassword());
        ...
        // 쿠키에 시간 정보를 주지 않으면 세션 쿠키(브라우저 종료 시 모두 삭제)
        Cookie idCookie = new Cookie("memberId", String.valueOf(loginMemer.getId()));
...
//    @PostMapping("/login")
    public String loginV2(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletResponse response) {
        ...
        sessionManager.createSession(loginMemer, response);
...
//    @PostMapping("/login")
    public String loginV3(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletRequest request) {
        ...
        // 로그인 성공 처리
        // 세션이 있으면 있는 세션을 반환, 없으면 신규 세션을 생성해서 반환해줌
        /*
            디폴트 값은 true
            request.getSession(true): 세션이 있으면 기존 세션을 반환, 세션이 없으면 세션을 생성해서 반환
            request.getSession(false): 세션이 있으면 기존 세션을 반환, 세션이 없으면 새로운 세션을 생성하지 않고 null을 반환
        */
        HttpSession session = request.getSession();
        // 세션에 로그인 회원 정보 보관
        session.setAttribute(SessionConst.LOGIN_MEMBER, loginMemer);
